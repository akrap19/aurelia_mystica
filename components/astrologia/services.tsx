import { Compass, Heart, Orbit, SunMoon, ArrowUpRight } from "lucide-react";

import { SERVICES_COPY, type Language } from "./copy";

const SERVICE_ICONS = [Compass, Heart, Orbit, SunMoon];
    ...
    >
      <div className="mb-16 flex flex-col items-center text-center md:mb-20">
        ...
        <h2 className="font-display text-aurelia-cream mt-6 max-w-2xl text-3xl leading-[1.15] font-light tracking-[0.05em] uppercase md:text-5xl">
          {copy.titleBefore}{" "}
          <span className="text-gold-gradient">{copy.titleAccent}</span>{" "}
          {copy.titleAfter}
        </h2>
      ...
      <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
        {copy.services.map((service, index) => {
          ...
          <article
            key={service.title}
            className="group border-gold-soft hover:border-aurelia-gold/60 from-aurelia-purple/25 relative flex flex-col overflow-hidden rounded-lg border bg-gradient-to-b to-transparent p-8 transition-all hover:-translate-y-1"
          >
            ...
            <span className="text-aurelia-gold/70 mt-2 font-serif text-xs tracking-[0.2em] uppercase italic">
              {service.duration}
            </span>
